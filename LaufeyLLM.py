...
max_iters = 3000
eval_interval = 300
learning_rate = 1e-2 #Could also be 1e-3, no problem with a dataset this small
device = 'cuda' if torch.cuda.is_available() else 'cpu' #Faster parallel processing if CUDA gpu is present
eval_iters = 200

with open('LaufeyLyrics.txt', 'r', encoding='utf-8') as f: #Opens the lyrics file in same directory
    text = f.read()
...
class BigramLanguageModel(nn.Module): #I don't really have any idea what's going on here
